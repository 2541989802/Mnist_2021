...
w1 = np.random.random((6,x.shape[0],3,3))
#w1 = w1*np.random.randint(-1,2,w1.shape)
b1 = np.random.randint(-1,2,(w1.shape[0],1,1))
w2 = np.random.random((12,w1.shape[0],3,3))
b2 = np.random.randint(-1,2,(w2.shape[0],1,1))
                        
r_w3 = w2.shape[0]*(int)((((int)(x.shape[1]-w1.shape[2]+2)/2)-w2.shape[2]+2)/2)*(int)((((int)(x.shape[2]-w1.shape[3]+2)/2)-w2.shape[3]+2)/2)
print(r_w3)
w3 = np.random.random((r_w3*1+0*R*R,R*R*R))*(10**-(1*R))
b3 = np.random.random((1,w3.shape[1]))
w4 = np.random.random((w3.shape[1],R*R*R))*(10**-(1*R))
...
def forword(x,w1,b1,w2,b2,w3,b3,w4,b4,w5,b5):
    ...
    #****forward full neural
    x3 = 1/(1+exp(-1*matmul(x_2,w3)+b3))
    x4 = 1/(1+exp(-1*matmul(x3,w4)+b4))
    x5 = 1/(1+exp(-1*matmul(x4,w5)+b5))
    return x1,x_1,x2,x_2,x_2_shape,x3,x4,x5

for interior in range(5000):
    y = np.array((1,2))
    x = np.array((5,5))
    ...
    x1,x_1,x2,x_2,x_2_shape,x3,x4,x5 = forword(x,w1,b1,w2,b2,w3,b3,w4,b4,w5,b5)
...
for interior in range(20):
    ...
    x1,x_1,x2,x_2,x_2_shape,x3,x4,x5 = forword(x,w1,b1,w2,b2,w3,b3,w4,b4,w5,b5)
    a = abs(x5-a)
    print(a,a[0,0]<0.5,a[0,1]<0.5,x5)
