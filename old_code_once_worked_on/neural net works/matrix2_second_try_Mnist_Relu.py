...
ans_pic = zeros((Batch,1,28,28))
_y = zeros((Batch))
y = zeros((Batch,1,10))

labels = tool.trainset_read_Batch(ans_pic)
tool.trainlabel_read_Batch(_y,labels)
print(ans_pic)
print(_y)
...
x1 = tool.relu(tool.convolute(ans_pic[0],w1)+b1)
...
for interior in range(Loop):
    ...
    for batch_i in range(Batch):
        ...
        x1 = tool.relu(tool.convolute(ans_pic[ans_id]/255,w1)+b1)
        x1_shape = x1.shape
        x1 = tool.conv3D_step(x1,(ones((x1.shape[0],2,2))/4),2,2)
        ...
        dCdw1 = tool.dw_convolute(dCdx1,ans_pic[ans_id]/255,w1);    _dCdw1=_dCdw1 + dCdw1/Batch;
...
for j in range(2):
    ...
    for i in range(Batch):
        ...
        c = abs(tool.softmax(x5)-y[ans_id])
        print("\n",j*Batch+i,"\n","\nx5:",x5,"\ny:",y[ans_id],"\nc:",c,"\nlabel:",labels[ans_id])
        if((c<0.5).all()):
            count = count + 1
        else:
