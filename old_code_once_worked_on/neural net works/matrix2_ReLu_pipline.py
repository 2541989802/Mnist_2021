...
def relu2D(n):
    ...
    for i in range(n.shape[0]):
        t = n[i]
        threads.append(threading.Thread(target=relu2D_h,args=(t,)))
...
def relu3D(m):
    ...
    for i in range(m.shape[0]):
        t = m[i]
        threads.append(threading.Thread(target=relu3D_h,args=(t,)))
    for thread in threads:
        thread.start()
    for thread in threads:
        thread.join()
...
def d_relu2D(n):
    ...
    for i in range(n.shape[0]):
        t = n[i]
        threads.append(threading.Thread(target=d_relu2D_h,args=(t,)))
    for thread in threads:
        thread.start()
    for thread in threads:
        thread.join()
...
def d_relu3D(m):
    ...
    for i in range(m.shape[0]):
        t = m[i]
        threads.append(threading.Thread(target=d_relu3D_h,args=(t,)))
    for thread in threads:
        thread.start()
    for thread in threads:
        thread.join()
...
def conv3D(m, f):
    ...
    temp = np.zeros((m.shape[0],m.shape[1]-f.shape[1]+1,m.shape[2]-f.shape[2]+1))
    threads = []
    for i in range(m.shape[0]):
        t = temp[i]
        threads.append(threading.Thread(target=conv3D_h,args=(temp[i],m[i],f[i],)))
    for thread in threads:
        thread.start()
    for thread in threads:
        thread.join()
...
def multiConvSum_h(s,m,ks,o):
    s = convTimesSum3DtoZ(m,ks,o)
...
def multiConvSum(m,ks,o):#m times o then convolute, each 3D kernal compose a whole 4D filter, m.shape[0]=(ks[i].shape)[0],ks.shape[0]==o.shape[0], m.shape[1]-ks.shape[2]+1==o.shape[1],m.shape[2]-ks.shape[3]+1==o.shape[2]
    ...
    threads = []
    for i in range(n):
        threads.append(threading.Thread(target=multiConvSum_h,args=(s[i],m,ks[i],o[i],)))
    for thread in threads:
        thread.start()
    for thread in threads:
        thread.join()
